...
MP_DISTRICTS = {
    ...
    "umaria": (80.8378, 23.5243),
    "anuppur": (81.6853, 23.1032),
    "mandsaur": (75.0672, 24.0718),
    "neemuch": (74.8720, 24.4730),
    "ratlam": (75.0367, 23.3315),
    "dewas": (76.0519, 22.9623),
    "dhar": (75.3025, 22.6013),
    "barwani": (74.9040, 22.0350),
    "khargone": (75.6144, 21.8255),
    "khandwa": (76.3529, 21.8264),
    "burhanpur": (76.2309, 21.3100),
    "betul": (77.8960, 21.9006),
    "harda": (77.0940, 22.3441),
    "hoshangabad": (77.7310, 22.7441),
    "sehore": (77.0854, 23.2030),
    "raisen": (77.7800, 23.3315),
    "vidisha": (77.8060, 23.5235),
    "guna": (77.3110, 24.6460),
    "ashoknagar": (77.7280, 24.5760),
    "shivpuri": (77.6550, 25.4238),
    "datia": (78.4598, 25.6653),
    "bhind": (78.7880, 26.5667),
    "morena": (78.6569, 26.4980),
    "sheopur": (76.6943, 25.6722),
    "balaghat": (80.1863, 21.8135),
    "mandla": (80.3739, 22.5983),
    "seoni": (79.5401, 22.0868),
    "chhindwara": (78.9395, 22.0574),
    "narsinghpur": (79.1925, 22.9467),
    "sidhi": (81.8820, 24.3956),
    "singrauli": (82.6780, 24.1990),
    "dindori": (81.0790, 22.9415),
    "alirajpur": (74.3610, 22.3130),
    "jhabua": (74.5970, 22.7670),
    "agar malwa": (76.0150, 23.7110),
    "shajapur": (76.2770, 23.4260),
    "rajgarh": (76.7280, 23.8700),
    "niwari": (78.8420, 25.3560),
    "pandhurna": (78.5260, 21.5950),
    "maihar": (80.7600, 24.2660)
}
CITY_COORDS.update(MP_DISTRICTS)

# Speed assumptions based on traffic level (km/h)
TRAFFIC_SPEED = {
    "low": 65,
    "medium": 50,
    "high": 35
...
def get_coords(city_name):
    ...
    # Otherwise geocode it using ORS
    try:
        url = "https://api.openrouteservice.org/geocode/search"
        headers = {"Authorization": ORS_API_KEY}
        params = {
...
def get_distance_eta(source, destination):
    ...
    try:
        ...
        distance    = round(segment["distance"] / 1000, 2)  # metres → km
        duration_min = int(segment["duration"] / 60)         # seconds → minutes
        eta = f"{duration_min // 60} hr {duration_min % 60} min"

        print(f"✅ ORS API: {source} → {destination} = {distance} km, ETA: {eta}")
        return distance, eta

    except Exception as e:
        print(f"❌ ORS API failed ({type(e).__name__}): {e}")

    # ── Fallback: CSV data + traffic-aware speed ───────────────────
    try:
        distance = get_distance_from_data(source, destination)
        if not distance:
            print(f"❌ No fallback distance found for {source} → {destination}")
            return None, None

        traffic   = get_traffic(source, destination)          # "low" / "medium" / "high"
        avg_speed = TRAFFIC_SPEED.get(traffic, 50)            # km/h
        total_minutes = int((distance / avg_speed) * 60)
        eta = f"{total_minutes // 60} hr {total_minutes % 60} min"

        print(f"⚠️ Fallback: {distance} km | Traffic: {traffic} | Speed: {avg_speed} km/h | ETA: {eta}")
        return distance, eta
